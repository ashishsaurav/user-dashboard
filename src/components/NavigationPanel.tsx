...
const NavigationPanel: React.FC<NavigationPanelProps> = ({
  ...
  const handleStartEditViewName = (view: View) => {
    setEditingViewId(view.id);
    setEditingViewName(view.name);
  };

  const handleEditViewNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setEditingViewName(e.target.value);
  };

  const handleEditViewNameSave = (view: View) => {
    if (editingViewName.trim() && editingViewName !== view.name) {
      const updatedViews = views.map((v) =>
        v.id === view.id ? { ...v, name: editingViewName } : v
      );
      onUpdateViews(updatedViews);
    }
                            ...
                            <div className="view-info">
                              ...
                              <div className="view-details">
                                {editingViewId === view.id ? (
                                  <input
                                    className="view-name-edit-input"
                                    value={editingViewName}
                                    autoFocus
                                    onChange={handleEditViewNameChange}
                                    onBlur={() => handleEditViewNameSave(view)}
                                    onKeyDown={(e) => {
                                      if (e.key === "Enter")
                                        handleEditViewNameSave(view);
                                      if (e.key === "Escape") {
                                        setEditingViewId(null);
                                        setEditingViewName("");
                                      }
